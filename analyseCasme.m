...
for didx=3:numel(dirs)
    ...
    for vidx=3:numel(viddir)
        ...
        stepSize = round(STEP_SEC*data.Fs);
        data.maxLags = 60/40; %Corresponding to 40bpm
        halfWin = (data.winLength/2);
        
        data.nWindows = numel(halfWin:stepSize:traceSize-halfWin-data.maxLags);
        ...
        if ~METHOD_SrPDE
            ...
            %% SrPDE processing
            data.videowriter = VideoWriter([viddir(vidx).folder '/' viddir(vidx).name   '/srpderaw.avi'],'Uncompressed AVI');
        ...
        prevprog=-1;
        linelength=[];
        for i=halfWin:stepSize:traceSize-halfWin
            if(ind==1)
                fprintf('\nAccumulating %d frames for the first window (total %d)\n',data.winLength,numel(halfWin:stepSize:traceSize-halfWin-data.maxLags));
            else
            ...
            if ind == 1 %First temporal window
                ...
                for j = startInd:endInd
                    ...
                    if useFaceLandmarks && j==1
                        ...
                        %% Uncomment the following block to verify faceroi
                        %figure,image(data.frame)
                        %hold on, plot(data.face_landmarks(:,1),data.face_landmarks(:,2),'.')
                    ...
                    if ~METHOD_SrPDE
                        ...
                        %% SrPDE
                       [data] = stepSrPDE(j,ind,startInd,stepSize,endInd,data);
...
function [data] = stepSrPDE(j,ind,startInd,stepSize,endInd,data)
if j==1
...
if(j==endInd)
    ...
    [R, C, L] = size(X_prj);
    for r= 1:R
        [rPPG_freq, rPPG_powers, rPPG_peaksLoc] = fftrPPG(squeeze(X_prj(r,:,:)),data.Fs,-1,true);
        for c = 1:C
    ...
    %Normalize SNRpx
    data.SNRpx = data.SNRpx - mean(mean(data.SNRpx));
    data.SNRpx = data.SNRpx/std(std(data.SNRpx));
...
%             end
    ...
    if (ind==data.nWindows+1 && j==endInd)
        %Flush out the remaining frames of the last window
%         for I=startInd + stepSize:data.nFrames
%              data.frameD = data.X_bar +(data.SNRpx.*repmat(data.sPulseTrace(:,:,I),1,1,3));
%         end
